...
# A tree that died halfway is the one thing that would ship silently
# broken, and an abort leaves one behind. Mark it, loudly, so a later
# stage cannot mistake it for a finished build -- and so a caller that
# swallowed the exit status (a pipe to `tail`, a CI step that only
# checks the last command) still has something to trip over.
FAILED_MARKER="$ROOTFS/.INCOMPLETE"
on_error() {
    local line=$1
    printf '\n\033[1;31m'
    printf '==============================================================\n'
    printf '  BUILD FAILED at line %s. The tree at\n' "$line"
    printf '    %s\n' "$ROOTFS"
    printf '  is INCOMPLETE and must not be imaged. Remove it and\n'
    printf '  run again once the cause is fixed:\n'
    printf '    sudo rm -rf %s\n' "$ROOTFS"
    printf '==============================================================\n'
    printf '\033[0m\n'
    [ -d "$ROOTFS" ] && date -Iseconds > "$FAILED_MARKER"
}
...
# ---------------------------------------------------------------------------
# Passed explicitly rather than relying on the caller's environment
# leaking through: whether the image carries Ollama is a visible build
# decision, not an ambient one.
OLLAMA_SKIP="${OLLAMA_SKIP:-0}" "$BUILD_DIR/scripts/install-inference.sh" "$ROOTFS"
...
# Unmount before measuring: du would otherwise walk into the bind-mounted
# /proc and count the host's kernel state -- and worse, race transient
# PIDs that vanish mid-scan, which fails the whole build on a line whose
# only job is to print a number.
cleanup
trap - EXIT
trap - ERR

# Only a run that reached this line produces a tree stage 2 will touch.
rm -f "$FAILED_MARKER"

log "Rootfs complete: $ROOTFS"
du -sh --one-file-system "$ROOTFS" | awk '{print "    size: " $1}'
cat <<EOF
